...
build -a AARCH64 -t CLANG38 -p edk2-platforms/Platform/Rockchip/RK$CHIP/RK$CHIP.dsc $FLAGS &&
...
cp $WORKSPACE/Build/RK$CHIP/DEBUG_CLANG38/FV/BL33_AP_UEFI.Fv unpack/uboot &&
./scripts/fit-repack.sh -f uboot_uefi.img -d unpack/
mv uboot_uefi.img $WORKSPACE/uboot_uefi.img
echo "****Build NOR FLASH IMAGE****"
cp $WORKSPACE/Build/RK$CHIP/DEBUG_CLANG38/FV/NOR_FLASH_IMAGE.fd $WORKSPACE/RK3588_NOR_FLASH.img
