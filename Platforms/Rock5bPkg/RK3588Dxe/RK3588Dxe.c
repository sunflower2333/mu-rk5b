...
EFI_STATUS
SetFramebufferRegionAttribute(VOID){
  ...
  if(EFI_ERROR(Status))
    DEBUG((EFI_D_WARN, "[DisplayLib]: CpuArchProtocol Not Found! %r", Status));

  Status = gCpu->SetMemoryAttributes (gCpu, 0x0C0000000, SIZE_128MB, EFI_MEMORY_WC | EFI_MEMORY_XP);

  if(EFI_ERROR(Status))
    DEBUG((EFI_D_WARN, "[DisplayLib]: Set Framebuffer Attributes failed! %r", Status));
